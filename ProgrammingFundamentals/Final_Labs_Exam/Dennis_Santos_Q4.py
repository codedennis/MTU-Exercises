def get_club_name():
    while True:
        club_name = input("Enter the name of the athletics club: ")
        if club_name.isalpha() and len(club_name) >= 2:
            return club_name
        else:
            print("Invalid club name. Please enter at least two alphabetical characters without whitespace or punctuation.")
...
def get_input():
    ...
    distance_covered = get_positive_float_input("How far did you run today (km): ")
    race_distance = get_positive_float_input("What distance race are you training for (km): ")
    rainfall_amount = get_positive_float_input("What is the hourly rainfall amount (mm): ")
